        ...
        public PollyHttpClient() =>
            Policy.Handle<HttpRequestException>()
                .OrResult<HttpResponseMessage>(response => !response.IsSuccessStatusCode)
                    .WaitAndRetryAsync(MAX_RETRY_ATTEMPTS, sleepProvider => WAIT_BEFORE_RETRY);

        private void OnRetry(DelegateResult<HttpRequestMessage> result, TimeSpan span, int retryCount, Context context) =>
